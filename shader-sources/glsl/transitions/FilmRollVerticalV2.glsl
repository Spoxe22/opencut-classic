// Organic vertical film-strip transition for portrait slideshows.
...
vec4 transition(vec2 uv) {
  ...
  float frame_shadow = rounded_box(q, frame_center + vec2(0.004, 0.018), image_half_size + vec2(0.020), 0.028);
  ...
  vec4 strip = vec4(FILM_BASE.rgb * mix(1.0, 0.58, edge_curve), 1.0);
  strip = mix(strip, FILM_EDGE, outer_frame);
  strip = mix(strip, BLACK, frame_shadow * (1.0 - image_mask) * 0.38);
